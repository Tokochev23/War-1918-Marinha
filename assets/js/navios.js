...
function updateShipCalculations() {
    ...
    let countryCostReduction = 0, countryProductionCapacity = 0;
    if (countryData) {
    ...
    if (!hullData) {
        ...
        // Limpa UI
        ['unit_cost', 'total_production_cost', 'displacement', 'build_time', 'max_speed', 'range', 'effective_armor', 'firepower', 'aa_capability', 'asw_capability', 'reliability_display', 'max_allowed_cost'].forEach(id => {
            document.getElementById(id).textContent = id === 'unit_cost' ? '£0' : '0';
        });
        return null;
    }
    ...
    if (doctrineData) {
        ...
        if(doctrineData.performance_bonus) {
            for(const key in doctrineData.performance_bonus) {
                if(performanceBonuses[key]) performanceBonuses[key] *= doctrineData.performance_bonus[key];
            }
        }
    ...
    if(mainGunCaliber > 0 && mainGunQuantity > 0) {
        if(mainGunCaliber > hullData.max_main_gun) {
            document.getElementById('main_gun_note').textContent = `Calibre máximo para ${hullData.name}: ${hullData.max_main_gun}mm`;
    ...
    let totalFirepower = mainGunCaliber * mainGunQuantity;
    ...
    // Equipamentos
    let totalASWCapability = depthCharges * 10;
    ...
    if(depthCharges > 0) {
        baseCost += shipData.components.armaments.depth_charge.cost * depthCharges;
        baseTonnage += shipData.components.armaments.depth_charge.tonnage * depthCharges;
    }

    // Mostra/esconde seção de aeronaves
    ...
    if(hullType === 'escort_carrier' || hullType === 'fleet_carrier') {
        ...
        baseCost += aircraftCapacity * 900000 / hullData.max_aircraft; // Custo proporcional
    } else {
    ...
    baseCost *= (1 + (sliderValue * 0.20));
    constructionTimeModifier *= (1 - (sliderValue * 0.30)); // Menos tempo se focar em velocidade
    
    document.getElementById('production_quality_note').textContent = 
        `Foco ${sliderValue > 0 ? 'em Velocidade' : 'em Qualidade'}: ` +
        `Confiabilidade ${sliderValue > 0 ? '-' : '+'} / Custo ${sliderValue > 0 ? '-' : '+'} / Tempo ${sliderValue > 0 ? '-' : '+'}`;
    ...
    // Velocidade
    const speedPenaltyFromWeight = Math.max(0.5, 1 - (finalTonnage / (hullData.tonnage * 3)));
    const finalSpeed = Math.round(baseSpeed * performanceBonuses.speed * speedPenaltyFromWeight);
    
    // Alcance
    const baseRange = 1000 + (finalTonnage * 2); // Alcance base proporcional ao tamanho
    ...
    } else if(finalSpeed < 15 && hullType !== 'submarine') {
        statusEl.textContent = "⚠️ Navio muito lento!";
        statusEl.className = "status-indicator status-warning";
    } else {
    ...
    return {
        ...
        hullType: hullData.name, engineType: engineType ? shipData.components.engines[engineType].name : '-',
        ...
        armamentDetails: {
            mainGuns: mainGunCaliber > 0 ? `${mainGunQuantity}x ${mainGunCaliber}mm` : 'Nenhum',
...
function collectSecondaryArmament() {
    ...
    ['aa_20mm', 'aa_37mm', 'aa_40mm', 'hybrid_90mm', 'hybrid_127mm', 'secondary_152mm'].forEach(id => {
        const qty = parseInt(document.getElementById(id).value) || 0;
        if(qty > 0) {
            const name = shipData.components.armaments[id].name;
            armaments.push(`${qty}x ${name}`);
        }
    });
...
function collectEquipment() {
    const equipment = [];
    document.querySelectorAll('#equipment-section input:checked').forEach(cb => {
        equipment.push(shipData.components.equipment[cb.id].name);
    });
    return equipment;
}
...
function saveShipDesign() {
    ...
    if(shipDesignData) {
        localStorage.setItem('shipDesignData', JSON.stringify(shipDesignData));
        
        // Abre a ficha em nova janela
        window.open('ficha_navio.html', '_blank');
...
// Função para carregar aeronaves salvas (integração)
function loadSavedAircraft() {
    ...
    savedAircraft.forEach((aircraft, index) => {
        ...
        div.innerHTML = `
            ...
            <span class="text-sm text-gray-600">${aircraft.date ? new Date(aircraft.date).toLocaleDateString() : ''}</span>
        `;
...
function selectAircraft(index) {
    ...
    document.querySelectorAll('.aircraft-item')[index].classList.add('selected');
    // Aqui você pode adicionar lógica adicional para vincular a aeronave ao porta-aviões
}
